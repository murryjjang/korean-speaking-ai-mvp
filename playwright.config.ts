...
export default defineConfig({
  ...
  projects: [
    ...
    // E2E smoke tests — requires Chromium (sudo npx playwright install-deps chromium on WSL2)
    {
      ...
      testMatch: ['**/mobile-speaking.spec.ts', '**/auth-routes.spec.ts', '**/reading-practice.spec.ts', '**/presentation-practice.spec.ts', '**/analytics-dashboard.spec.ts', '**/pdf-download.spec.ts', '**/console-errors.spec.ts'],
    },
  ],

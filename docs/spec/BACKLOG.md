...
**상태(enum):** `pending`
**의존성:** D-014 A안(`hrefs` prop 주입) 출시 후.
**시점:** 시연 후 (정식 운영 진입 시)

**배경:** 야간5에서 1.6 배너를 research 흐름에 wire-in하며 `DashboardBanner`에 `hrefs?` prop 주입(A안, 소변경)으로 해결(D-014). 흐름이 늘면 호출처마다 `lang`·`hrefs`·`vocabDueCount` 주입이 반복되고, 흐름별 동선 차이(예: research엔 SRS/vocab 미적용)가 호출부에 흩어진다.
...
**해야 할 일:**
- **학습 시작 위치 일관성** — 모바일(상단 mobile-top)/데스크톱(하단) 분기 정리(현 `단계19.13·19.14` 우회 흡수). research↔정식 학생 흐름 간 위치 통일. **D-015 흡수**: 학습 시작이 단일 위치가 되면, 배너 '오늘의 학습' 앵커의 dual-instance 우회(`data-scroll-target` 2곳 + offsetParent 가시성 판정)를 단일 `id="start-learning"` 으로 단순화.
- **페이지 layout 통일** — 헤더·카드·섹션 패턴을 흐름 무관하게 표준화.
- **디자인 토큰·컴포넌트 표준화** — 토큰/공용 컴포넌트 정비, **Day 0 leftover** 일부 흡수.
**관련:** D-014(배너 wire-in 시 드러난 흐름별 위치 분기) · BL-#8.
